...
def main():
    ...
    if os.path.exists(args.dir+'/bundled_links') == False:
        try:
          ...
          p = subprocess.check_output('./bundler -l '+ args.dir+'/contig_links -o ' + args.dir+'/bundled_links + -b '+args.dir+'/bundled_graph.gml', shell=True)
    ...
    if os.path.exists(args.dir+'/repeats') == False:
      try:
       #os.system('./vc_algo -g '+ args.dir+'/bundled_graph.gml -r ' + args.dir+'/repeats -f 0.02')
        p = subprocess.check_output('./vc_algo -g '+ args.dir+'/bundled_graph.gml -r ' + args.dir+'/repeats -f 0.02',shell=True)
      except subprocess.CalledProcessError as err:
      ...
      try:
        ...
        p = subprocess.check_output('python remove_repeats.py -l '+ args.dir+'/bundled_links -r ' + args.dir+'/repeats -o '+ args.dir+'/bundled_links_filtered -c '+args.dir+'/contig_coverage -s '+args.dir+'/contig_length',shell=True)
      except subprocess.CalledProcessError as err:
        print >> sys.stderr, time.strftime("%c")+': Failed to find repeats, terminating scaffolding....\n' + str(err.output)
        sys.exit()
    ...
    if os.path.exists(args.dir+'/scaffolds.fasta') == False:
      ...
      try:
        p = subprocess.check_output('python layout.py -a '+ args.assembly + ' -g ' + args.dir+'/oriented.gml -s '+args.dir+'/seppairs -o '+args.dir+'/scaffolds.fa -f '+args.dir+'/scaffolds.agp -b '+args.dir+'/bubbles',shell=True)
